...
// room.src.mjs
var room_src_default = async (req) => {
  const headers = { "content-type": "application/json", "cache-control": "no-store" };
  try {
    const store = getStore({ name: "crawl", consistency: "strong" });
    if (req.method === "GET") {
      const v = await store.get("state");
      return new Response(v || "null", { headers });
    }
    if (req.method === "PUT" || req.method === "POST") {
      const body = await req.text();
      if (!body || body.length > 9e5) return new Response('{"error":"bad body"}', { status: 400, headers });
      JSON.parse(body);
...
var config = { path: "/api/room" };
export {
  config,
  room_src_default as default
};
